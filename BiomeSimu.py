...
def bisection2D(xa,xb,eps, mu, M):
    """For the purposes of comparing the speed of the bisection method with the orginal and the new system"""
    p1 = 0
    p2 = 1
    while np.linalg.norm(p2 - p1) > eps:
        po = (p2 + p1)/2.0
...
def sep_check(filename1, filename2):

  
    with open('data/{}'.format(filename1), 'rb') as f:
        sep_list_2D, sep_list_11D = pickle.load(f)
#        print(sep_list_2D.get((6,3))+sep_list_2D.get((3,0)))
#        print(sep_list_11D.get((6,3))+sep_list_11D.get((3,0)))
#        print(sep_list_2D.get((6,4))+sep_list_2D.get((4,0)))
#        print(sep_list_11D.get((6,4))+sep_list_11D.get((4,0)))
        print('----------------------')
        print(sep_list_11D.get((6,3)))
        print(sep_list_11D.get((3,0)))
        print('-----------------------')
        print(sep_list_11D.get((6,4)))
        print(sep_list_11D.get((4,0)))
    with open('data/{}'.format(filename2), 'rb') as f:
        print('-----------------------')
        sep_list_2D, sep_list_11D = pickle.load(f)
        print(sep_list_11D.get((25,15)))
        print(sep_list_11D.get((15,0)))
        print('-----------------------')
        print(sep_list_11D.get((25,20)))
        print(sep_list_11D.get((20,0))) 
        print('-----------------------')
...
def hammingLists(ordered_paths_2D,ordered_paths_11D,inpt): 
    ...
    print('OrderedLists{}.txt'.format(inpt))
...
#    print('NORMED ------------------------------------')
#    for key in ordered_paths_2D:
        #prints path seqeunces (e.g. 1221, 1222, 1223)
#        print(key)
#        print('  ', ordered_paths_2D[key])
#        print('  ', path_lengths_2D[key])
#        print('  ', ordered_paths_11D[key])
#        print('  ', path_lengths_11D[key])
...
#        print('  ', ordered_paths_11D[key])
        ...
        if '60' in path_lengths_11D and count == 0:
            print('60')
            print('  ', path_lengths_11D['60'])
            count = 1
        if '610' in path_lengths_11D and count1 == 0:
            print('610')
            print('  ', path_lengths_11D['610'])
            count1 = 1
        if '630' in path_lengths_11D and count2 == 0:
            print('630')
            print('  ', path_lengths_11D['630'])
            count2 = 1
#        hd = hamming_distance(ordered_paths_2D[key], ordered_paths_11D[key])
##        print('   hamming distance:', hd)
##        total_hamming_distance += hd
...
def CompareUnstableDirections(UD1,UD2):
    with open('OrderedListsUnitary Unstable1.txt', "rb") as f:
        myobj = pickle.load(f)
        print(myobj)
...
def main():
    ...
    if trackstates:
        ...
        for i in range(len(criteria)):
            criteria[i] = options[optionInput[2]][i]
...
#                p = [tuple(translation_dict[State2][State1][x] for x in pp) for pp in ordered_paths_11Ds[State2][path_UD_2][:3]]
        ...
        # Stein : 0.0 %
        # 1-UD : 0.0 % (All states & Non-none states)
        # 2-UD : 5.0 %
        ...
        # Stein : 0.0%
        # 1-UD : 22.2% (Non-none states) --> 13.3% (All states)
        # 2-UD : 30.0%
...
#            p = ordered_paths_11Ds[State1][path][:3]
        ...
        # 2-UD : 45.0 % (All states & Non-none states)
        #How often does the most optimal path between two stein steady states change when using 2 unstable directions? (e.g. 2 out of 20 times there is a "None" introduced)
        # Stein : 0.0 %
        # 1-UD : 25 % -- >  15% (All states)
        # 2-UD :20% (All states & Non-none states)
        #How often do the three most optimal paths between two stein steady states change when using 2 unstable directions? (e.g. 10 out of 60 times there is a "None" introduced)
        # Stein : 0.0%
        # 1-UD : 44.4% (Non-none states) --> 26.7% (All states)
        # 2-UD : 56.7% (All states & Non-none states)
        #Repeat these two things using: 
        #most optimal path between 1-UD steady states when including 2-UD steady states
        #How often does the optimal path change?
        # 50 % of the time a 2-UD steady state in the most optimal path
        #How often do the three most optimal paths change? 
        #  62.5 % of the time a 2-UD steady state in the three most optimal path
        #most optimal path between Stein steady states when including 1-UD steady states (* this will be kind of tricky, since one of the Stein steady states is 2-UD and so this might not work very well)
